(function() {
 ...
 // Set the API secret key BEFORE any ImageDB operation
...
if (storedKey) {
  ...
  // Pre‑fill the input with the stored value
  ...
  if (setBtn) {
    setBtn.addEventListener('click', function() {
      ...
      if (!newKey) {
        showAlert('Please enter a valid secret key.'); // reuse your existing modal
        return;
      }
      
      // Save to localStorage and configure ImageDB
  ...
  return `${month}, ${year}`;
      ...
      function syncPathFromDropdowns() {
        ...
        renderDropdowns();               // ★ re‑render to add/remove deeper levels
      ...
      function renderDropdowns() {
        ...
        // Build a dropdown for each depth from root up to one beyond current path
        for (let i = 0; i <= currentPath.length; i++) {
          ...
          //  sep.className = 'text-cyan-400/50 text-xs';
      ...
      function updateFolderUpBtn() {
        ...
        folderUpBtn.onclick = () => {
          ...
          renderDropdowns();               // ★ also re‑render after going up
          renderGalleryFromExplorer();
          updateFolderUpBtn();
        };
      }
  ...
  summaryLast.textContent = '—';
        ...
        galleryGrid.innerHTML = files.map((img, idx) => {
          ...
          //const dateText = img.uploadedAt ? new Date(img.uploadedAt).toLocaleDateString() : '';
          // const dateText = img.uploadedAt ? formatToMonthYear(img.uploadedAt) : '';
        ...
        // Card click → slideshow (using ImageDB slideshow, but with current folder files)
      ...
      function initExplorer(images, storeId) {
        ...
        // Start at root – no deep hardcoded path
...
/**
 * Resize & compress an image file if it exceeds a max size (in bytes).
 * Returns a Promise that resolves to a new File (or the original if already small).
 */
// Helper (add this once, anywhere in your script)
function compressIfLarge(file) {
  ...
  return new Promise(resolve => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(blob => {
        // ★ Preserve the original file date here ★
        resolve(new File([blob], file.name, {
          type: 'image/jpeg',
          lastModified: file.lastModified   // ← THIS LINE
        }));
      }, 'image/jpeg', 0.85);
    };
...
// Updated upload button (replaces your one-liner)
...
apiEndpointDisplay.textContent = ImageDB.getApiBase();
  ...
  // Compress only those that exceed 4.5 MB (others remain unchanged)
  const processedFiles = await Promise.all(
    files.map(file => compressIfLarge(file))
  );
  
  // Build categories from the preserved lastModified dates
